...
Custom themes: copy `source/XeniaManager/Resources/Themes/Template.axaml` to a new file, recolor it (keep all `x:Key` names), register the name in the `Theme` enum (`source/XeniaManager.Core/Models/Theme.cs`) and the theme dictionary (`ThemeService.cs`) - full steps are in the main repo's Contributing Guide and summarized in [Development](../help/development.md#themes).
